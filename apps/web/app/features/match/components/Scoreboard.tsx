...
export default function Scoreboard({
  ...
  return (
    <div className="w-full bg-gray-900/50 backdrop-blur-sm rounded-2xl p-4 border border-gray-700/50 shadow-inner overflow-hidden mb-6 relative">
      <div
        className={`flex ${isMultiplayer ? "flex-col gap-4" : "justify-between items-center"} w-full`}
      >
        {/* Players Container */}
        <div
        ...
        {/* Round Counter */}
        <div
          className={`${isMultiplayer ? "w-full pt-2 border-t border-gray-800/50" : "absolute left-1/2 -translate-x-1/2 flex flex-col items-center"} text-center`}
        >
          <p className="text-[9px] text-gray-500 uppercase tracking-[0.2em] font-iosevka-bold mb-0.5">
            Round
          </p>
          <div className="flex items-center justify-center gap-1.5">
            <span className="text-xl font-iosevka-bold text-white/90">
              {currentRound}
            </span>
            <span className="text-gray-600 font-iosevka-light text-sm">/</span>
            <span className="text-sm font-iosevka-medium text-gray-400">
              {maxRounds}
            </span>
          </div>
        </div>
